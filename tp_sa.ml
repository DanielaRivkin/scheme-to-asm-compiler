...
module Reader : READER = struct
  ...
  and nt_paired_comment str = 
    let nt1 = caten (char '}') (caten (star nt_sexpr) (char '}')) in
    unitify nt1 str
  ...
  and make_skipped_star (nt : 'a parser) =
    ...
    nt1
  and nt_digit str = 
    let nt1 = pack (range '0' '9') (let d = int_of_char '0' in 
      fun n -> int_of_char n - d) in
    nt1 str  

  and nt_hex_digit str = 
    let nt1 = pack (range 'a' 'f') (let d = int_of_char 'a' - 10 in 
      fun n -> int_of_char n - d) in 
    let nt2 = pack (range 'A' 'F') (let d = int_of_char 'A' - 10 in 
      fun n -> int_of_char n - d) in 
  ...
  and nt_nat str =
    ...
    let nt1 = pack nt1 (fun d -> List.fold_left
       (fun n d -> n * 10 + d) 0 d) in
    nt1 str
  ...
  and make_maybe nt none_value =
    pack (maybe nt)
      (function
       ...
       | Some(x) -> x)
  and maybe_expo str = 
    let nt1 = pack (maybe nt_exponent) (function
                                        | Some(e) -> e 
                                        | None -> 1.0) in
    nt1 str
     
  and maybe_mant str =
    let nt1 = pack (maybe nt_mantissa) (function 
                                        | Some(m) -> m 
                                        | None -> 0.0) in
    nt1 str
  
  and floatA str = 
    let nt1 =  caten (caten (caten nt_integer_part (char '.')) maybe_mant) maybe_expo in
    let nt1 = pack nt1 (fun (((n,_),m),e)->ScmReal((n+.m)*.e)) in 
    nt1 str

  and floatB str = 
    let nt1  = caten (caten (char '.') nt_mantissa) maybe_expo in
    let nt1 = pack nt1 (fun ((_,m),e)->ScmReal(m*.e)) in
    nt1 str
  
  and floatC str =
    let nt1 = caten nt_integer_part nt_exponent in
    let nt1 = pack nt1 (fun (n,e)-> ScmReal(n*.e)) in
    nt1 str

  and sign_check str =
    let nt1 = pack (maybe (one_of "+-")) ( fun (s) -> match s with
                                          | Some (a) -> if a = '-' then -1.0 else 1.0
                                          | None -> 1.0) in                      
    nt1 str
  and nt_float str = 
    let nt1 = disj_list ([floatA; floatB; floatC]) in
    let nt1 = caten sign_check nt1 in
    let nt1 = pack nt1 (fun (s,a)->(match a with
                        | ScmReal(n) -> ScmReal(s*.n)
                        | ScmRational(t,b)-> ScmRational(( (truncate s)*t),b))) in
    nt1 str
  ...
  and nt_symbol str =
    ...
    let nt1 = pack nt1 (fun ch -> ScmSymbol ch) in
    nt1 str
  ...
  and nt_string_part_dynamic str =
    let nt1 = caten (unitify (caten nt_skip_star (char '~'))) 
                    (unitify (caten (char '{' ) nt_skip_star)) in
    let nt1 = caten (caten nt1 nt_sexpr) (unitify (char '}')) in
    let nt1 = pack nt1 (fun ((_,s),_) -> Dynamic(ScmPair (ScmSymbol ("format") , ScmPair ( ScmString("~a"), ScmPair(s , ScmNil ))))) in
    nt1 str
  ...
  and nt_vector str =
    let nt1 = word "#(" in
    let nt2 = caten (unitify (star (disj (unitify nt_whitespace) nt_comment))) (char ')') in
    let nt2 = pack nt2 (fun _ -> ScmVector []) in
    let nt3 = plus nt_sexpr in
    let nt1 = caten nt1 (disj nt2 (pack (caten nt3 (char ')'))(fun (s, _) -> ScmVector s))) in
    let nt1 = pack nt1 (fun (_, s) -> s) in
    nt1 str

  and nt_list str = 
    let nt1 = pack (char ')') (fun _ -> ScmNil) in
    let nt2 = pack (caten (char '.') (caten nt_sexpr (char ')'))) (fun (_,(s,_)) -> s) in
    let nt2 = disj nt1 nt2 in
    let nt2 = caten (plus nt_sexpr) nt2 in
    let nt2 = pack nt2 (fun (es, s) -> List.fold_right (fun a b -> ScmPair (a, b)) es s) in
    let nt1 = pack (caten nt_skip_star (char ')')) (fun (_,_) -> ScmNil) in
    let nt1 = caten (char '(') (disj nt1 nt2) in
    let nt1 = make_skipped_star (pack nt1 (fun (_, s) -> s)) in
    nt1 str
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_qq = function
    ...
    let rec macro_expand_and_clauses expr = function
      | [] -> expr
      | expr' :: exprs -> 
        let then_branch = macro_expand_and_clauses expr' exprs in
        let else_branch = ScmBoolean false in
        ScmPair (ScmSymbol "if", ScmPair (expr, ScmPair (then_branch, ScmPair (else_branch, ScmNil))))
  ...
  let rec or_macro_exp expr = function 
    | [] ->  expr
    | expr' :: exprs ->  ScmPair (ScmSymbol "if",
      ScmPair (expr, ScmPair (expr, ScmPair (or_macro_exp expr' exprs,ScmNil))))
  let get_vars ribs =
    let rec aux acc = function
      | ScmNil -> acc
      | ScmPair (ScmPair (ScmSymbol var, _), ScmNil) ->
        if is_reserved_word var then raise (X_syntax "reserved word")
        else ScmPair (ScmSymbol var, acc)
      | ScmPair (ScmPair (ScmSymbol var, _), ribs') ->
        let acc' = if is_reserved_word var then acc else ScmPair (ScmSymbol var, acc) in
        aux acc' ribs'
      | _ -> raise (X_syntax "malformed ribs expression") in
    let rev_vars = aux ScmNil ribs in
    let rec reverse acc = function
      | ScmNil -> acc
      | ScmPair (hd, tl) -> reverse (ScmPair (hd, acc)) tl
      | _ -> raise (X_syntax "malformed variable list")
    in
    reverse ScmNil rev_vars
  
  let rec get_vals ribs =
    let rec aux acc = function
      | ScmNil -> acc
      | ScmPair (ScmPair (_, ScmPair (expr, ScmNil)), ScmNil) ->
        ScmPair (expr, acc)
      | ScmPair (ScmPair (_, ScmPair (expr, ScmNil)), ribs') ->
        aux (ScmPair (expr, acc)) ribs'
      | _ -> raise (X_syntax "malformed ribs expression")
    in
    let rev_vals = aux ScmNil ribs in
    let rec reverse acc = function
      | ScmNil -> acc
      | ScmPair (hd, tl) -> reverse (ScmPair (hd, acc)) tl
      | _ -> raise (X_syntax "malformed value list")
    in
    reverse ScmNil rev_vals

  let rec build_set_expr ribs exprs =
    match ribs with
    | ScmNil -> exprs
    | ScmPair (ScmPair (ScmSymbol var, ScmPair (value, ScmNil)), rest_ribs) ->
      let reserved = is_reserved_word var in
      if reserved then
        raise (X_syntax "reserved word")
      else if rest_ribs = ScmNil then
        ScmPair (ScmPair (ScmSymbol "set!", ScmPair (ScmSymbol var, ScmPair (value, ScmNil))), exprs)
      else
        build_set_expr rest_ribs (ScmPair (ScmPair (ScmSymbol "set!", ScmPair (ScmSymbol var, ScmPair (value, ScmNil))), exprs))
    | _ -> raise (X_syntax "malformed ribs expression")

  let rec init_vals ribs = 
    let rec aux ribs acc = 
      match ribs with
      | ScmNil -> List.rev acc
      | ScmPair (ScmPair (ScmSymbol var, ScmPair (value, ScmNil)), rest) ->
          if is_reserved_word var then raise (X_syntax "reserved word")
          else aux rest ((ScmPair (ScmSymbol var, ScmPair (ScmSymbol "'whatever", ScmNil))) :: acc)
      | _ -> raise (X_syntax "malformed ribs expression")
    in
    let rec from_list lst = 
      match lst with
      | [] -> ScmNil
      | hd :: tl -> ScmPair (hd, from_list tl)
    in from_list (aux ribs [])
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
      tag_parse (ScmPair((ScmPair (ScmSymbol "lambda", ScmPair ((get_vars ribs), exprs))), (get_vals ribs)))
    ...
    ScmPair (ScmPair (ScmPair (var,
                      ...
                      ribs),
              exprs)) -> 
                let rib = ScmPair (ScmPair (var, ScmPair (arg, ScmNil)), ScmNil) in
                let body =
                  let let_star = ScmSymbol "let*" in
                  let ribs_exprs = ScmPair (ribs, exprs) in
                  ScmPair (ScmPair (let_star, ribs_exprs), ScmNil) in
                let exp = ScmPair (ScmSymbol "let", ScmPair (rib, body)) in
                tag_parse exp 

    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
        tag_parse (ScmPair (ScmSymbol "let", ScmPair((init_vals ribs), (build_set_expr ribs exprs))))
    ...
    | ScmPair (ScmSymbol "or", exprs) -> 
      (match (scheme_list_to_ocaml exprs) with
      | expr :: exprs, ScmNil ->
          tag_parse (or_macro_exp expr exprs)
      | _ -> raise (X_syntax "malformed and-expression"))
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmSeq exprs -> ScmSeq' (List.map (fun ex -> (run ex params  env)) exprs)
      | ScmOr exprs -> ScmOr' (List.map (fun ex -> (run ex params env)) exprs) 
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function
      ...
      | (ScmBox' _) as expr' -> (run in_tail expr')
      | (ScmBoxGet' _) as expr' ->  (run in_tail expr')
  ...
  let should_box_var name expr params = 
    let num_reads, num_writes = List.length (fst (find_reads_and_writes name expr params)), List.length (snd (find_reads_and_writes name expr params)) in
    if num_reads = 0 || num_writes = 0 then
      false
    else if num_reads = 1 && num_writes = 1 then
      (List.hd (fst (find_reads_and_writes name expr params))) <> (List.hd (snd (find_reads_and_writes name expr params)))
    else
      true
  ...
  let semantics expr =
    auto_box(annotate_lexical_address expr);;
  ...
  | ScmApplic' (proc, args, app_kind) ->
     ...
     let args =
       Reader.scheme_sexpr_list_of_sexpr_list
         (List.map sexpr_of_expr' args) in
     ScmPair (proc, args)
  | _ -> raise (X_this_should_not_happen "debug");;
